...
// Implements the Delaunay triangulation
struct Delaunay {
    // list of coordinates such as `[[x0,y0,T0], [x1,y1,T1], [x2,y2,T2], [x3,y3,T3]]`
    // where `T[i]` are the values (e.g., temperatures) at that coordinate and will be used for interpolation
    vector<vector<double>> coordinates;

    // list of connectivity (topology) such as `[[0,2,1], [2,1,0]]`
    vector<vector<size_t>> triangles;

    // Returns a new Delaunay triangulation from a "cloud" of points
    // `cloud` is a list of coordinates such as `[[x0,y0,...], [x1,y1,..], ...]`
    static std::unique_ptr<Delaunay> make_new(vector<vector<double>> const& cloud, bool verbose) {
        // constants
